...
export const articles = mysqlTable("articles", {
	...
	// Editorial provenance. `publishedAt` is when BrentDesk published the
	// piece and must stay truthful; `eventDate` is when the underlying
	// development actually happened (the announcement, award, filing or
	// commissioning). An archive covering events that predate the
	// publication needs both, and must never conflate them.
...
(table) => [
	...
	index("idx_articles_event_date").on(table.eventDate),
