#!/usr/bin/env python3
"""Apply the final 19 article-body corrections while preserving locked metadata."""
...
RESULTS_SOURCE = Path("/home/ubuntu/apply_final_editorial_corrections.json")
RESULTS_COPY = BASE / "qa" / "final-correction-results.json"
PACKETS = BASE / "qa" / "final-correction-briefs"
DRAFTS = BASE / "drafts" / "raw"
LOG = BASE / "qa" / "final-correction-consolidation.json"
...
def apply(record: dict) -> dict:
    ...
    if corrected.get("sequence") != current.get("sequence") or corrected.get("slug") != current.get("slug"):
        raise ValueError(f"identity changed for sequence {sequence}")
    ...
    current["editorial_notes"] = corrected.get("editorial_notes") or "Final factual and editorial corrections applied; draft requires human review."
    target = next(DRAFTS.glob(f"{sequence:03d}-*.json"))
    target.write_text(json.dumps(current, ensure_ascii=False, indent=2) + "\n")
    return {"sequence": sequence, "slug": current["slug"], "path": str(target.resolve()), "reported_word_count": output["word_count"], "reported_h2_count": output["h2_count"], "workflow_status": output["workflow_status"]}
